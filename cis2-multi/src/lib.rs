...
#[init(contract = "CIS2-Multi", parameter = "InitParams")]
...
mod tests {
    ...
    fn initial_state<S: HasStateApi>(state_builder: &mut StateBuilder<S>) -> State<S> {
        let mut state = State::empty(state_builder, PublicKeyEd25519([0; 32]));
        state.mint(
            &TOKEN_0,
            &{
    ...
    fn test_mint() {
        ...
        let state = State::empty(&mut state_builder, PublicKeyEd25519([0; 32]));
        let mut host = TestHost::new(state, state_builder);
        let crypto_primitives = TestCryptoPrimitives::new();
        // Call the contract function.
        let result: ContractResult<()> =
            contract_mint(&ctx, &mut host, &mut logger, &crypto_primitives);
